// ModbusTest.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
uint16_t putASCIItoRTU(uint8_t* rbuf, uint8_t* abuf, uint8_t len)
{
	...
	{
		//crc
		uint16_t SendCrc = CalCRC16(rbuf,rlen - 2);
		INT16ToCh(&rbuf[rlen - 2], SendCrc);
		rlen += 2;
		return rlen;
	}
	return 0;
...
int main()
{
	uint8_t Tx2Buf[50] = {0x01, 0x06, 0x00,0x00,0x00 ,0x00};

	int16_t SendCrc;
	int16_t len = 2;
	if (0)
	{
		INT16ToCh(&Tx2Buf[len], 0x00F2);
		len += 2;
		//FloatToCh(&Tx2Buf[len], 200);
		//len += 4;
		INT16ToCh(&Tx2Buf[len], 1);
		len += 2;
	}
	else
	{
		...
		Tx2Buf[len++] = 4;
		/*
		INT16ToCh(&Tx2Buf[len], 7);
		len += 2;
		*/
		/*
		FloatToCh(&Tx2Buf[len],7.2);
		len += 4;
		*/
		
		FloatToCh(&Tx2Buf[len],88.2);
		len += 4;
		
		/*
		putDouble(&Tx2Buf[len], 0.0);
		len += 8;
		*/
	}

	SendCrc = CalCRC16(Tx2Buf,len);
	INT16ToCh(&Tx2Buf[len], SendCrc);
	len += 2;
